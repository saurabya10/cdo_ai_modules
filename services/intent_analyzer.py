...
class IntentAnalyzer:
    ...
    INTENTS = {
        ...
        "sal_troubleshoot": "User wants to troubleshoot SAL (Secure Analytics and Logging) event streaming from firewall devices",
    ...
    def _get_api_key(self):
        """Get API key using OAuth2 flow - same as working code"""
        url = "https://id.cisco.com/oauth2/default/v1/token"
        payload = "grant_type=client_credentials"
        
        value = base64.b64encode(
            f"{self.config.llm.client_id}:{self.config.llm.client_secret}".encode("utf-8")
        ).decode("utf-8")
